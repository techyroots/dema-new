...
module.exports = {
    ...
    async addReview(sellerId, sellerOldJson, reviewerId, reviewText, rating, shopperOldJson, productId) {
        try {
            const [isReviewAdded, isShopperReviewAdded] = await Promise.all([sellerUtils.addReview(sellerOldJson, reviewerId, reviewText, rating, shopperOldJson.name, productId)
                , sellerUtils.addBuyerReview(shopperOldJson, sellerId, reviewText, rating, sellerOldJson.name, productId)]);
    ...
    ,
    async response(dataJson, id, buyerId, reviewerType, reviewText, reviewerId, productId) {
        try {
            ...
            const [responsedataJson, buyerDataJson] = await Promise.all([
                sellerUtils.addResponse(dataJson, buyerId, reviewText, reviewerType, reviewerId, OldJson.name),
                Contract.isBuyerIdExist(Number(buyerId)).then(shopperData => IpfsService.gateway(shopperData).then(getShopperJSON => sellerUtils.addBuyerResponse(JSON.parse(getShopperJSON), id, reviewText, reviewerType, reviewerId, OldJson.name))),
            ]);
            const [sellerInfo, shopperInfo] = await Promise.all([
                common.uploadSeller(responsedataJson, id),
                common.uploadShopper(buyerDataJson, buyerId)
            ]);
            const saveHash = await Contract.addReviewReply(productId, id, buyerId, "P" + productId, sellerInfo[0], shopperInfo[0], 'AllProduct', sellerInfo[1], shopperInfo[1]);
            if (saveHash) {
                return { success: true, message: "Posted SuccessFully", data: "", error: "" };
            } else {
                return { success: false, message: "Can't process your request right now please try again later", data: "", error: "ERROR Product 10" };
